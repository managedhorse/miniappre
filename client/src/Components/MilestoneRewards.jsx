    ...
    <div className="flex flex-col w-full space-y-4">
      ...
      {milestones.filter(milestone => !claimedMilestones.includes(milestone.name)).map((milestone) => {
        ...
        return (
            ...
            <>
            <div key={milestone.name} className='bg-[#ffffff1a] rounded-[10px] px-[14px] py-[5px] flex flex-wrap justify-between items-center'>
...
<button
 ...
 onClick={() => handleClaim(milestone)}
  className={` ${isClaimable ? 'bgs-1 text-white' : "bg-btn2 text-[#fff6]"} relative rounded-[8px] font-semibold py-2 px-3`}>
